    ...
    <>
      {imgUrl?.redirect_link?.link_type === "external_url" ? (
        <Link className="h-100" href={imgUrl?.redirect_link?.link || "/"} target="_blank">
          {bgImage ? <div className={`bg-size ${classes}`} style={{ backgroundImage: "url(" + (resolveImg(imgUrl, placeholder)) + ")" }}></div> : <Image src={resolveImg(imgUrl, placeholder)} className="bg-img w-100 img-fluid" alt="banner" height={height} width={width} unoptimized style={{ objectFit: "cover", width: "100%", height: "100%" }} />}
        </Link>
        ...
        <div className={`bg-size ${classes}`} style={{ backgroundImage: `url(${resolveImg(imgUrl, placeholder)}` }}></div>
      ) : (
        (imgUrl?.image_url || placeholder) && <Image src={resolveImg(imgUrl, placeholder)} className="bg-img w-100 img-fluid" alt="banner" height={height} width={width} unoptimized style={{ objectFit: "cover", width: "100%", height: "100%" }} />
      )}
    </>
  );
};
